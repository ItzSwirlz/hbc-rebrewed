...
static char *_get_elem_cdata(mxml_node_t *node, const char *element) {
    ...
    return _get_cdata(
        mxmlFindElement(node, node, element, NULL, NULL, MXML_DESCEND_FIRST));
}
...
static bool _get_gradient(mxml_node_t *node, const char *element,
    ...
    u32 i, c[4];
    const char *const corner[] = {"upper_left", "upper_right", "lower_right",
                                  "lower_left"};
...
// sync with font.h
static const char *targets[FONT_MAX] = {"label",       "dlgtitle", "memo",
                                        "appname",     "appdesc",  "button",
                                        "button_desel"};
...
static char *_get_args(u16 *length, mxml_node_t *node, const char *element) {
    *length = 0;
    ...
    for (n = mxmlFindElement(node, node, "arg", NULL, NULL, MXML_DESCEND_FIRST);
         n != NULL;
         n = mxmlFindElement(n, node, "arg", NULL, NULL, MXML_NO_DESCEND)) {
        arg = _get_cdata(n);
...
meta_info *meta_parse(char *fn) {
    ...
    if (s) {
        ...
        } else if (sscanf(s, "%4d%2d%2d", &t.tm_year, &t.tm_mon, &t.tm_mday) ==
                   3) {
            t.tm_hour = 1;
            t.tm_min = 0;
            t.tm_sec = 0;
...
void theme_xml_init(void) { memset(&theme, 0, sizeof(theme)); }
...
bool load_theme_xml(char *buf) {
    ...
    while (fnode != NULL) {
        _get_font(fnode);
        fnode =
            mxmlFindElement(fnode, root, "font", NULL, NULL, MXML_NO_DESCEND);
    }

#ifdef USE_MUSIC
    mnode =
        mxmlFindElement(node, root, "music", NULL, NULL, MXML_DESCEND_FIRST);
    if (mnode != NULL) {
        _get_theme(mnode);
        mnode = mxmlFindElement(mnode, root, "music", NULL, NULL,
                                MXML_DESCEND_FIRST);
    }
#endif

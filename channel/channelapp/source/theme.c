...
#define _ENTRY_WS(n, w, h, w_ws, fn)                                           \
    {n##_png, &n##_png_size, w, h, n##_wide_png, &n##_wide_png_size, w_ws, fn}
...
static const struct {
    ...
    const char *filename;
} theme_data[THEME_LAST] = {ENTRY_WS(background, 640, 480, 852),
    ...
    for (i = 0; i < FONT_MAX; i++) {
        ...
        if (file) {
            ...
            for (j = 0; j < FONT_MAX; j++) {
                if (theme_fonts[j].file &&
                    !strcasecmp(theme_fonts[j].file, file) &&
                    theme_fonts[j].data) {
...
static void theme_load(u8 *data, u32 data_len) {
    ...
    for (i = 0; i < gi.number_entry; ++i) {
        res = unzGetCurrentFileInfo(uf, &fi, filename, sizeof(filename), NULL,
                                    0, NULL, 0);
        if (res != UNZ_OK) {
            gprintf("unzGetCurrentFileInfo failed: %d\n", res);
            goto error;
        ...
        if ((fi.uncompressed_size > 0) &&
            ...
            if (!strcasecmp(filename, theme_fn_xml)) {
                ...
                theme_gfx[index] =
                    tex_from_png(buf, fi.uncompressed_size, w, h);
            }
    ...
    for (i = 0; i < THEME_LAST; ++i) {
        if (!theme_gfx[i]) {
            ...
            if (widescreen && theme_data[i].data_ws)
                theme_gfx[i] = tex_from_png(
                    theme_data[i].data_ws, *(theme_data[i].size_ws),
                    theme_data[i].width_ws, theme_data[i].height);
            else
                theme_gfx[i] =
                    tex_from_png(theme_data[i].data, *(theme_data[i].size),
                                 theme_data[i].width, theme_data[i].height);
        }
...
bool theme_is_valid_fn(const char *filename) {
    ...
    if (l >= 5 && tolower((unsigned char)filename[l - 1]) == 'f' &&
    ...
    if (l >= 5 && tolower((unsigned char)filename[l - 1]) == 't' &&
